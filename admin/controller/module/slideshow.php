...
class ControllerModuleSlideshow extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$data['success'] = true;
			$this->response->redirect($this->url->link('module/slideshow', 'token=' . $this->session->data['token']."&module_id=37", 'SSL'));
		}
		...
		$data['entry_control'] = $this->language->get('entry_control');
		...
		if (isset($this->request->post['control'])) {
			$data['control'] = $this->request->post['control'];
		} elseif (!empty($module_info)) {
			$data['control'] = $module_info['control'];
		} else {
			$data['control'] = '';
		}
